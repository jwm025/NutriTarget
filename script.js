...
const dinners = [
  ...
  {
    name: "Garlic Butter Steak & Potatoes Skillet",
    cal: 493,
    p: 27,
    c: 40,
    f: 9,
...
const activityLevels = [
  ...
  {
    value: 1.9,
    text: "Extremely active (hard daily exercise and physical job)",
  },
];
...
function calculate() {
  ...
  if (
    ...
    alert(
      "Please fill in all required fields correctly. Age must be 18-78, weight/height must be positive and in range"
    );
    return;
  }
  ...
  if (units === "metric" && (weight < 20 || weight > 250)) {
    ...
    units !== "metric" &&
    (parseFloat(document.getElementById("lb").value) < 50 ||
      parseFloat(document.getElementById("lb").value) > 500)
  ) {
...
function generateMealPlan() {
  ...
  const totalP = Math.round(
    bMeal.p * bScale + lMeal.p * lScale + dMeal.p * dScale + sMeal.p * sScale
  );
  const totalC = Math.round(
    bMeal.c * bScale + lMeal.c * lScale + dMeal.c * dScale + sMeal.c * sScale
  );
  const totalF = Math.round(
    bMeal.f * bScale + lMeal.f * lScale + dMeal.f * dScale + sMeal.f * sScale
  );

  let planHTML = `
    <p>Breakfast: ${bMeal.name} (${bScale.toFixed(
    1
  )} portions) - ${bCal} cal, ${Math.round(bMeal.p * bScale)}g protein, 
    ${Math.round(bMeal.c * bScale)}g carbs, ${Math.round(
    bMeal.f * bScale
  )}g fat</p>
    <p>Lunch: ${lMeal.name} (${lScale.toFixed(
    1
  )} portions) - ${lCal} cal, ${Math.round(lMeal.p * lScale)}g protein, 
    ${Math.round(lMeal.c * lScale)}g carbs, ${Math.round(
    lMeal.f * lScale
  )}g fat</p>
    <p>Dinner: ${dMeal.name} (${dScale.toFixed(
    1
  )} portions) - ${dCal} cal, ${Math.round(dMeal.p * dScale)}g protein, 
    ${Math.round(dMeal.c * dScale)}g carbs, ${Math.round(
    dMeal.f * dScale
  )}g fat</p>
    <p>Snack: ${sMeal.name} (${sScale.toFixed(
    1
  )} portions) - ${sCal} cal, ${Math.round(sMeal.p * sScale)}g protein, 
    ${Math.round(sMeal.c * sScale)}g carbs, ${Math.round(
    sMeal.f * sScale
  )}g fat</p>
  `;
